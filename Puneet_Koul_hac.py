...
def read_input(input_file,documents_metastore,word_across_docs,total_docs):
	...
	#print total_docs,maxWords
	vector_list=csc_matrix((total_docs,maxWords), dtype=float)
	#for key in documents_metastore:
	#	temp_list=documents_metastore.get(key)
...
def get_unit_vector(document,documents_metastore,word_across_docs,document_count,vocab_words,total_words,vector_list,merged_vectors_map,magnitude):
	...
	for word in doc_info:
		...
		#print "done"
		#vector_tf_idf[int(word[0])]=tf_idf
	...
	#print "Here in get unit vector"
	#print euclidean_distance,int(document)
	...
	for col in range(0,size[1]):
		...
		#row = np.array([col])
		#col = np.array([int(document)])
		#data = np.array([normalised_tfidf])
...
def take_average(merged_vectors,documents_metastore,vector_list,merged_vectors_map,new_merged_vectors_map,document_count):
	...
	#print v12
	#if isinstance(merged_vectors[0], int):
	#	v1=vector_list[int(merged_vectors[0])]
	#else:
	#	doc_no=new_merged_vectors_map.get(merged_vectors[0])
	#	v1=vector_list[int(doc_no)]

	#if isinstance(merged_vectors[1], int):
	#	v2=vector_list[int(merged_vectors[1])]
	#else:
	#	doc_no=new_merged_vectors_map.get(merged_vectors[1])
	#	v2=vector_list[int(doc_no)]
	#v12=[]
	#for i in range(len(v1)):
	#	v12.append(float(v1[i]+v2[i])/2)

	#v12=map(lambda a, b: float(a + b)/2, v1, v2)

	#for i in len(merged_vectors):
	#	merged_vectors[i]
	#	v1=vector_list[int(merged_vectors[0])]
	value_list=[]
	for i in range(len(v12)):
		if v12[i] !=0.0:
			...
			value_list.append(value)
	#merged_vectors=convert_to_one_cluster(merged_vectors)
	#print "doc_metastore in ",merged_vectors,value_list
	documents_metastore[merged_vectors]=value_list
	size=vector_list.shape
	vector_list=vector_list.toarray()
	vector_list = np.vstack([vector_list,v12])
	#vector_list.indptr = np.hstack((vector_list.indptr,vector_list.indptr[-1]))
	#print vector_list.toarray()

	#vector_list[document_count]=v12
	document_count+=1
...
def calculate_other_cs(merge_docs,vector_list,merged_vectors_map,new_merged_vectors_map,documents_metastore,input_heap,clusters):
	size=vector_list.shape
	#temp_list=list(merge_docs)
	#print "do this ==================================================",merge_docs,clusters
	for i in range(size[0]):
		if i in merged_vectors_map:
			...
			if tuple(already_merged) not in clusters or tuple(already_merged) == merge_docs:
				#print "ooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo",tuple(already_merged)
				pass
			else:
				...
				#print "b ",i,merged_vectors_map.get(i)
...
def already_clustered(current_docs,popped_clusters):
	...
	if tup1 not in popped_clusters and tup2 not in popped_clusters:
		#print "Valid and Merged"
		#print "\n"
		return True
	else:
		#print "Invalid and left out"
		#print "\n"
		return False
...
#vector_list=np.zeros((int(document_count),max(word_across_docs.keys())+1))
#vector_list=csc_matrix((int(document_count),max(word_across_docs.keys())+1), dtype=np.float).toarray()
outsidecount=0
...
for vector_pair in itertools.combinations(range(0,int(document_count)),2):
	...
	if len(similar_words)!=0:
		...
		cos_similarity=calculate_cosine_similarity(a,b,mag_a,mag_b,similar_words)
		if cos_similarity !=0.0:
			#print "cosine similairity",cos_similarity,int(vector_pair[0]),int(vector_pair[1])
			val=(1.0-float(cos_similarity),vector_pair)
			#print val
...
for i in v_1[0]:
	...
	count+=1
#print "\n"
#print "doc 1 unit vector:",dum
#print "\n"
#print "doc 2 unit vector:  ",dum1
...
while(True):
	...
	## take care of non exissitng in clustering 
	#print "Trying to Merge",(1.0-float(merge_docs[0])),merge_docs[1]
	dummy=merge_docs
	merge_docs=merge_docs[1]
	#print "check here --------------------------------",merge_docs
	#,popped_clusters
	if(already_clustered(merge_docs,popped_clusters)):
		...
		#print "\n"
		#print " Merging Valid -> Cosine  : ",(1.0-float(dummy[0]))," Pair : ",dummy[1]
		#print "\n"
